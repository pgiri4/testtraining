...
def generate_my_events(filename, n_events):
    ...
    ########### FOR Emitter ###############
    data_sets = {}
    data_sets["emitter_antenna_type"] = ["RNOG_vpol_4inch_center_n1.73"]*n_events #["XFDTD_Vpol_CrossFeed_150mmHole_n1.78"]*n_events#["bicone_v8_inf_n1.78"] * n_events
    data_sets["emitter_model"] = ["ARA02_calPulser"] * n_events
    ...
    data_sets["xx"] = np.ones(n_events)* 10 * units.m
    data_sets["yy"] = np.ones(n_events) * 12 * units.m
    data_sets["zz"] = -np.ones(n_events) * 97 * units.m
